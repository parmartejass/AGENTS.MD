...
class Application(tk.Tk):
    def __init__(
        self,
        *,
        job_executor: Callable[..., object],
        scenario_loader: Callable[[Path], object] = load_scenario,
    ) -> None:
        super().__init__()
    ...
    def _on_run(self) -> None:
        ...
        try:
            scenario = self._scenario_loader(scenario_path)
    ...
    def _worker_run(self, scenario_id: str, job_config) -> None:
        result = self._job_executor(job_config, cancel_event=self._cancel_event, mode="gui")
...
def start_gui(job_executor: Callable[..., object]) -> None:
    app = Application(job_executor=job_executor)
    app.mainloop()
