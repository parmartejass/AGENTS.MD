...
Ownership follows `AGENTS.md` FP-10, FP-12, and FP-22. This document defines the config boundary and validation mechanics. For explicitly declared and validated doc-owned keys, defaults or schemas, apply `docs/agents/25-docs-ssot-policy/docs-ssot-policy.md` Rule: Declared owners own facts and Bounded Project Authority Memory; its Baseline placement routes project records while the Config authority package below remains the config contract.
...
- Authority boundary: the owning module, config file, schema, external artifact, or explicitly owned doc; which facts it owns; and which surfaces are examples, local overrides, or consumers only.
...
- App identity and filename contract: the config owner MUST declare filename derivation from the app identity owner or the authoritative external format. `<APP_NAME>.json` is an example, not a default. The config owner declares identity consumption, filename, runtime-root derivation, and development/packaged location rules. Consumers MUST NOT hardcode location rules outside that contract.
...
- Runtime location/name authority: the owner or derivation rule for config path, filename, and runtime root. Config may own these facts only when that is its declared jurisdiction; otherwise it must consume the owning runtime-path authority.
...
Repo-owned shared platform settings route to `docs/agents/settings/00-settings-standards/settings-standards.md`; user-local secrets and machine-local overrides remain outside repo-owned config packages unless a future owner explicitly adopts them.
...
- Config drift detection: when config values can become stale relative to runtime conditions, the config owner MUST expose a staleness witness and reject stale resolution under `AGENTS.md` FP-27 unless its governing contract explicitly permits it.
