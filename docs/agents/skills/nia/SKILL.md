...
> Repo `.env` files are optional local conveniences only. The bundled shell scripts read the current process environment or `~/.config/nia/api_key`; they do not auto-load `.env`.
...
For a saved Remote MCP + rules setup for Claude Code, Cursor, and Codex, see `SETUP.md`.
...
SLACK_BOT_TOKEN=xoxb-... ./scripts/slack.sh register-token [name] # Register external bot token (BYOT)
...
2. Or use BYOT: `SLACK_BOT_TOKEN=xoxb-your-token ./scripts/slack.sh register-token "My Workspace"`
...
DB_CONNECTION_STRING=... ./scripts/folders.sh from-db <name> <query>
DB_CONNECTION_STRING=... ./scripts/folders.sh preview-db <query>
```
