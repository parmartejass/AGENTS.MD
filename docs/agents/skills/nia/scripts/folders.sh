...
cmd_from_db() {
  if [ -z "$2" ]; then
    echo "Usage: folders.sh from-db <name> <query>"
    echo "  Env: DB_CONNECTION_STRING, TABLE, DB_TYPE, COLUMNS"
    return 1
  fi
  if [ -z "${DB_CONNECTION_STRING:-}" ]; then
    echo "Error: Set DB_CONNECTION_STRING in the environment."
    return 1
  fi
  DATA=$(jq -n \
    --arg name "$1" --arg conn "$DB_CONNECTION_STRING" --arg query "$2" \
...
cmd_preview_db() {
  if [ -z "$1" ]; then
    echo "Usage: folders.sh preview-db <query>"
    echo "  Env: DB_CONNECTION_STRING, TABLE, DB_TYPE, COLUMNS, LIMIT"
    return 1
  fi
  if [ -z "${DB_CONNECTION_STRING:-}" ]; then
    echo "Error: Set DB_CONNECTION_STRING in the environment."
  ...
  DATA=$(jq -n \
    --arg conn "$DB_CONNECTION_STRING" --arg query "$1" \
    --arg table "${TABLE:-}" --arg dbtype "${DB_TYPE:-}" --arg cols "${COLUMNS:-}" \
    --arg limit "${LIMIT:-5}" \
