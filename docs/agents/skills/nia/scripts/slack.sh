...
cmd_register_token() {
  if [ -z "${SLACK_BOT_TOKEN:-}" ]; then
    echo "Usage: SLACK_BOT_TOKEN=<xoxb-bot-token> slack.sh register-token [workspace_name]"
    return 1
  fi
  DATA=$(jq -n --arg token "${SLACK_BOT_TOKEN}" --arg name "${1:-}" \
    '{bot_token: $token}
...
cmd_messages() {
  ...
  if [ -n "${2:-}" ]; then url="${url}&channel=$(urlencode "$2")"; fi
  nia_get "$url"
}
