...
cmd_tree() {
  ...
  if [ -n "${INCLUDE_PATHS:-}" ]; then params="${params}&include_paths=$(urlencode "$INCLUDE_PATHS")"; fi
  if [ -n "${EXCLUDE_PATHS:-}" ]; then params="${params}&exclude_paths=$(urlencode "$EXCLUDE_PATHS")"; fi
  if [ -n "${FILE_EXTENSIONS:-}" ]; then params="${params}&file_extensions=$(urlencode "$FILE_EXTENSIONS")"; fi
  if [ -n "${EXCLUDE_EXTENSIONS:-}" ]; then params="${params}&exclude_extensions=$(urlencode "$EXCLUDE_EXTENSIONS")"; fi
