...
- If mixed-owner runtime paths remain allowed, record the decision in `docs/agents/22-ssot-authority-decisions.md`.

### Stage 2 - Checker authority consolidation
- Status: completed
- Align `scripts/check_governance_core.py` with `scripts/check_change_records.ps1` for governance council-summary validation, including `intent_coverage`.
...
- Verification outcome:
  - `python3 scripts/check_python_safety.py` now reports `0 error(s), 0 warning(s)`
  - `python3 scripts/check_governance_core.py --fail-on-safety-warnings` now passes
